import { getExercises } from '@cwt/api/exercisesService';
...
export async function asyncGetExercises(
  token: string,
): Promise<Exercise[] | void> {
  try {
    const exercises = await getExercises(baseUrl, token);
    console.log('Mobile: Exercises recieved from backend: ', exercises);
    return exercises;
  } catch {
    console.log(
      'Mobile: Error fetching exercises from API. Sample exercises set for development.',
    );
